...
export class ModerationService extends IMService {
	...
	public async addStrikesAndPunish(
		...
		const strikesBefore = await this.strikes.strike(guild, user, type, amount, modAndReason, extras);
		if (amount > 0) {
			const strikesAfter = strikesBefore + amount;

			const punishmentConfigs = await this.punishmentsCache.get(guild.id);
			const punishmentConfig = punishmentConfigs.find((c) => c.amount > strikesBefore && c.amount <= strikesAfter);

			await this.punishments.punish(guild, user, punishmentConfig.type, amount, modAndReason, [], extras);
		}
	}
