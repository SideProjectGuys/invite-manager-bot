...
export default class extends Command<IMClient> {
  ...
  public async action(message: Message, [role, invites, description]: [Role, number, string]): Promise<any> {
    ...
    await ranks.insertOrUpdate({
      ...
      if (!created) {
        message.channel.send(`Rank ${role.toString()} updated: Now needs ${invites} and has the following description: "${description ? description : ''}"`);
      } else {
        message.channel.send(`Added rank ${role.toString()} which needs ${invites} and has the following description: "${description ? description : ''}"`);
      }
    });
  }
}
