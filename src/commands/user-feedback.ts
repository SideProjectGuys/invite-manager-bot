  ...
  public async action(message: Message, feedback: [string]): Promise<any> {
    ...
    if (config.feedbackChannel) {
      ...
      channel.send({ embed: embedFeedback }).then(() => {
        ...
        embed.setDescription('Thank you for your feedback! If you need personal assistance please join our support discord server.');
        embed.addField('Links', `[Support Discord](${config.botSupport}) | [Invite this bot to your server](${config.botAdd}) | [Website](${config.botWebsite})`);
